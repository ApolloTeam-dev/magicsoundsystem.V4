...
std::map<uint8_t, std::string> ApolloSoundChannels;
...
{
    #ifdef APOLLO
	ADXX(sprintf(ApolloDebugMessage, "MSS_OpenWave: Opening WAV file: %s\n", name);)
...
{
	#ifdef APOLLO
	ADXX(sprintf(ApolloDebugMessage, "MSS_SetLog: Setting log file\n");)
	ADXX(ApolloDebugPutStr(ApolloDebugMessage);)	
	#endif
...
{
    #ifdef APOLLO
	ADXX(ApolloDebugPutStr("AudioCallback_OGG called\n");)
	// Apollo Audio does not use Callback
	return;
	#endif
...
{
    #ifdef APOLLO
	ADXX(ApolloDebugPutStr("AudioCallback_MP3 called\n");)
	// Apollo Audio does not use Callback
	return;
	#endif
...
{
	#ifdef APOLLO
	ADXX(ApolloDebugPutStr("AudioCallback_WAV called\n");)
	// Apollo Audio does not use Callback
	return;
	#endif
...
{
    #ifdef APOLLO
	AD(ApolloDebugPutStr("AudioCallback_Midi called\n");)
	// Apollo Audio does not use Callback
	return;
	#endif
...
{
    #ifdef APOLLO
	AD(sprintf(ApolloDebugMessage, "MSS_SetAudioSystemLoaders: Setting audio system loaders\n");)
	AD(ApolloDebugPutStr(ApolloDebugMessage);)
	#endif    
...
{
	#ifdef APOLLO
	ADXX(sprintf(ApolloDebugMessage, "CheckAHIAudioMode: Checking AHI audio mode\n");)
	ADXX(ApolloDebugPutStr(ApolloDebugMessage);)
	return true;
	#endif
...
{
    #ifdef APOLLO
	AD(sprintf(ApolloDebugMessage, "MSS_SoundInit\n");)
	AD(ApolloDebugPutStr(ApolloDebugMessage);)
	#endif
	...
	struct dirent *apollo_direntry;
	DIR *apollo_dir = opendir("Apollo");
	if (apollo_dir)
	{
		while ((apollo_direntry = readdir(apollo_dir)) != NULL)
		{
			if ( (strstr(apollo_direntry->d_name,".aiff")) && (strstr(apollo_direntry->d_name,"00")) )
			{
				struct ApolloSound apollo_sound;
				char apollo_filename[256];
				strcpy(apollo_filename, "Apollo/");
				strcat(apollo_filename, apollo_direntry->d_name);
				strcpy(apollo_sound.filename, apollo_filename);
				apollo_sound.format = APOLLO_AIFF_FORMAT;
				apollo_direntry->d_name[strlen(apollo_direntry->d_name) - 5] = 0; // Remove file extension for cache map key

				uint8_t result = ApolloLoadSound(&apollo_sound);
				if(result != 0)
				{
					AD(sprintf(ApolloDebugMessage, "Failed to load sound %s, error code %d\n", apollo_sound.filename, result);)
					AD(ApolloDebugPutStr(ApolloDebugMessage);)
				}
				else
				{
					ApolloCopyBlock(apollo_sound.buffer+apollo_sound.position, ApolloSoundCache+ApolloCachePointer, apollo_sound.size);

					strcpy(ApolloSoundCacheMap[apollo_direntry->d_name].filename, apollo_filename);
					ApolloSoundCacheMap[apollo_direntry->d_name].buffer 		= ApolloSoundCache;
					ApolloSoundCacheMap[apollo_direntry->d_name].position 		= ApolloCachePointer;
					ApolloSoundCacheMap[apollo_direntry->d_name].size 			= apollo_sound.size;
					ApolloSoundCacheMap[apollo_direntry->d_name].period 		= apollo_sound.period;
					ApolloSoundCacheMap[apollo_direntry->d_name].volume_left 	= 127;
					ApolloSoundCacheMap[apollo_direntry->d_name].volume_right 	= 127;
					ApolloSoundCacheMap[apollo_direntry->d_name].loop 			= false;

					AD(sprintf(ApolloDebugMessage, "Pre-Cached in ApolloSoundCache[%-12s]: File=%-25s | Size=%8d | Cache=%12d | Period=%5d \n",
						 apollo_direntry->d_name, ApolloSoundCacheMap[apollo_direntry->d_name].filename, ApolloSoundCacheMap[apollo_direntry->d_name].size,
						  ApolloCachePointer, ApolloSoundCacheMap[apollo_direntry->d_name].period);)
					AD(ApolloDebugPutStr(ApolloDebugMessage);)

					ApolloCachePointer += apollo_sound.size;	
		...
		for(auto& entry : ApolloSoundCacheMap)
		{
			AD(sprintf(ApolloDebugMessage, "Testing Cached Sound: %s | Size: %d | Position: %d\n", entry.second.filename, entry.second.size, entry.second.position);)
			AD(ApolloDebugPutStr(ApolloDebugMessage);)
				
			int8_t result = ApolloPlaySound(&entry.second);
			if(result != 0)
			{
				AD(sprintf(ApolloDebugMessage, "Failed to play cached sound %s, error code %d\n", apollo_sound.filename, result);)
				AD(ApolloDebugPutStr(ApolloDebugMessage);)
			}
...
{
	#ifdef APOLLO
	AD(sprintf(ApolloDebugMessage, "MSS_SoundClose\n");)
	AD(ApolloDebugPutStr(ApolloDebugMessage);)
	#endif
	...
	#ifdef APOLLO
	AD(ApolloDebugPutStr("MSS_SoundClose: Freeing Apollo sound cache\n");)
...
extern "C" void MSS_Play(void *handle, double _vol, double _pan, int looped, bool ignoreGlobalVolume)
{
	...
	if(sound) // && (strstr((char*)sound->extFile, "00")) && ((looped!=0) && !sound->playing) || (looped==0))
	{
		...
		for(uint8_t channel =0; channel < 16; channel++)
		{
			...
			if(channelfree) ApolloSoundChannels[channel] = "";
			
			if(ApolloSoundChannels[channel] == (char*)sound->extFile)
			{
				ADXX(sprintf(ApolloDebugMessage, "MSS_Play       : Sound %s is already playing on channel %d\n", (char*)sound->extFile, channel);)
				ADXX(ApolloDebugPutStr(ApolloDebugMessage);)
				return;
			}
		}
		...
		struct ApolloSound apollo_sound;
		strcpy(apollo_sound.filename, (char*)sound->extFile);
		...
		uint8_t result=ApolloPlaySound(&apollo_sound);
		if(result != 0)
		{
			AD(sprintf(ApolloDebugMessage, "MSS_Play       : Failed to play sound %s, error code %d\n", apollo_sound.filename, result);)
			AD(ApolloDebugPutStr(ApolloDebugMessage);)
		} else {
			ApolloSoundChannels[apollo_sound.channel] = apollo_sound.filename;
		}
...
extern "C" void MSS_Stop(void *handle)
{
	...
	if(sound)
	{
		...
		if(apollo_sound.size > 20000000)
		{
			...
			AD(strcpy(apollo_sound.filename, (char*)sound->extFile);)
...
{
	#ifdef APOLLO
	AD(sprintf(ApolloDebugMessage, "MSS_LoadStreamFromMemory: Loading stream from memory\n");)
	AD(ApolloDebugPutStr(ApolloDebugMessage);)
	#endif
...
{
	#ifdef APOLLO
	AD(sprintf(ApolloDebugMessage, "MSS_LoadWave: Loading wave file %s\n", name);)
	AD(ApolloDebugPutStr(ApolloDebugMessage);)
	return 0;
	#endif	
...
{
    #ifdef APOLLO
	// Apollo Audio is always bases on DMA from memory
	return 0;
	#endif
...
extern "C" void *MSS_LoadSample(const char* name)
{
	...
	if(ApolloSoundCacheMap.find(name) == ApolloSoundCacheMap.end())
	{
		strcpy(apollo_filename, "Apollo/");
		...
		if(result != 0)
		{
			AD(sprintf(ApolloDebugMessage, "MSS_LoadSample : Failed to load sound %s, error code %d\n", apollo_sound.filename, result);)
			AD(ApolloDebugPutStr(ApolloDebugMessage);)
		}
		else
		{
			AD(sprintf(ApolloDebugMessage, "MSS_LoadSample : CACHE MISS !!! -> Loaded Sound from DISK into ApolloSoundCache: %s, size %d bytes, period %d\n", apollo_sound.filename, apollo_sound.size, apollo_sound.period);)
			AD(ApolloDebugPutStr(ApolloDebugMessage);)
			
			ApolloCopyBlock(apollo_sound.buffer+apollo_sound.position, ApolloSoundCache+ApolloCachePointer, apollo_sound.size);

			strcpy(ApolloSoundCacheMap[name].filename, apollo_filename);
			ApolloSoundCacheMap[name].buffer 		= ApolloSoundCache;
			ApolloSoundCacheMap[name].position 		= ApolloCachePointer;
			ApolloSoundCacheMap[name].size 			= apollo_sound.size;
			ApolloSoundCacheMap[name].period 		= apollo_sound.period;
	...
	ADXX(sprintf(ApolloDebugMessage, "MSS_LoadSample : File=%-25s | Size=%8d | Cache=%12d | Period=%5d \n",
...
{
	#ifdef APOLLO
	AD(sprintf(ApolloDebugMessage, "MSS_LoadStreamToSoundFX: Loading stream to sound effect from file %s\n", filename);)
	AD(ApolloDebugPutStr(ApolloDebugMessage);)
	#endif
